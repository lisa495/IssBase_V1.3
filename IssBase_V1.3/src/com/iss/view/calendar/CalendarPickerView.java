...
import com.iss.view.calendar.MonthCellDescriptor.RangeState;
...
public class CalendarPickerView extends ListView {
    ...
    public FluentInitializer init(Date minDate, Date maxDate) {
        ...
        while ((monthCounter.get(MONTH) <= maxMonth // Up to, including the
            ...
            LogUtil.d("Adding month %s", month);
            months.add(month);
        ...
        switch (selectionMode) {
            case RANGE:
                if (selectedCals.size() > 1) {
                    ...
                    // We're moving the start of the range back in time: clear
                    // the old start date.
                    clearOldSelections();
                }
                break;
    ...
    List<List<MonthCellDescriptor>> getMonthCells(MonthDescriptor month, Calendar startCal) {
        ...
        while ((cal.get(MONTH) < month.getMonth() + 1 || cal.get(YEAR) < month.getYear()) //
            ...
            LogUtil.d("Building week row starting at %s", cal.getTime());
            ...
            for (int c = 0; c < 7; c++) {
                ...
                weekCells.add(new MonthCellDescriptor(date, isCurrentMonth, isSelectable,
                        isSelected, isToday, value, rangeState));
                cal.add(DATE, 1);
            }
        }
        return cells;
