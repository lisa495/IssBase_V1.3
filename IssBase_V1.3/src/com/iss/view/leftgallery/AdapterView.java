...
// CHECKSTYLE:ON
     ...
     * {@link android.view.View.OnCreateContextMenuListener#onCreateContextMenu(
    ...
    public final void addView(final View child, final int index) {
        throw new UnsupportedOperationException("addView(View, int) is "
     ...
     * or {@link #INVALID_POSITION} if there is nothing selected.
     */
    ...
    final int findSyncPosition() {
        ...
        // Get the item ID locally (instead of getItemIdAtPosition), so we need the adapter
        final T adapter = getAdapter();
        if (adapter == null) {
            return INVALID_POSITION;
        }
